...
// ::: Code Generated by Copilot 3e9a71c2-8d54-4b0a-b6f1-2a7c9e4d5f83. This comment will be removed automatically after the file is saved :::
...
export async function hydrateSharedProjects() {
  ...
  const clauses = ["name='" + DRIVE.projectFile + "'", 'sharedWithMe=true', 'trashed=false'];
  const url =
    DRIVE_FILES + '?q=' + driveQuery(clauses) +
    '&spaces=drive&fields=files(id,name,parents,owners(displayName,emailAddress))';
  const response = await driveFetch(url);
  ...
  const projects = await Promise.all(
    files.map(async (file) => {
      ...
      const project = {
        ...
        slug: data.slug || '',
        folderId: (Array.isArray(file.parents) && file.parents[0]) || data.folderId || null,
        fileId: file.id,
